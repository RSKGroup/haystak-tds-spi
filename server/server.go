...
func (s *Server) handle(conn net.Conn) {
	...
	sess, princ, err := s.handshake(conn)
	if err != nil {
		s.logf("handshake error: %v", err)
		return
	}
	s.logf("handshake complete (user=%q)", princ.Username)
	s.serve(sess, princ)
}

func (s *Server) handshake(conn net.Conn) (net.Conn, tds.Principal, error) {
	...
	if err := s.send(conn, wire.BuildLoginResponse(s.serverName(), s.database())); err != nil {
		return nil, none, err
	}
	s.logf("sent LOGIN response")
	return conn, princ, nil
}
...
func (s *Server) serve(conn net.Conn, princ tds.Principal) {
...
func (s *Server) runQuery(ctx context.Context, sql string) ([]byte, error) {
	rows, affected, err := engine.Exec(ctx, s.Backend, sql)
	if err != nil {
		return nil, err
	}
	if rows == nil {
		if affected >= 0 {
			return wire.DoneWithCount(uint64(affected)), nil
		}
		return wire.EmptyDone(), nil
	}
	defer rows.Close()
	cols := rows.Columns()
	var data [][]any
	...
	return wire.BuildResultResponse(cols, data)
}
