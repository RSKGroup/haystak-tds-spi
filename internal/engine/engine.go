...
func runParsed(ctx context.Context, b tds.Backend, q *tds.Query) (tds.Rows, error) {
	...
	if caps.Aggregate && len(q.Joins) == 0 && q.FromSub == nil && len(q.GroupingSets) == 0 && !exec.HasDistinctAgg(q) && exec.IsAggregate(q) {
...
func isProbeScalar(name string) bool {
	...
	case "SYSTEM_USER", "CURRENT_USER", "SESSION_USER", "USER", "USER_NAME", "SUSER_NAME", "SUSER_SNAME",
		"HOST_NAME", "APP_NAME", "SCHEMA_NAME", "SERVERPROPERTY", "CURRENT_TIMESTAMP":
		return true
	}
	return false
}
