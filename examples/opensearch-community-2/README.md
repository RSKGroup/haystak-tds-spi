# examples/opensearch-community-2

Same idea as [`examples/opensearch-community`](../opensearch-community) (serve a live
OpenSearch cluster over the SQL Server wire), but with a **declared** catalog instead of a
purely inferred one — adapted to a store that already has a schema.

OpenSearch is not schemaless: every index has a native `_mapping`. So this example is a
...
OpenSearch already knows the fields and their types, so re-declaring them by hand (as the
Mongo [`mongodb-community-2`](../mongodb-community-2) example must) would be busywork. What
...
> The reserved index is `haystak_catalog`, not Mongo's `__haystak_catalog`: OpenSearch
> index names may not start with `_`.

## Why this is better than inference on OpenSearch

- Types are correct, not guessed. Sampling can mis-type a column (an integer that's null in
  the first 100 docs, a field that's `keyword` in the mapping but looks like free text). The
...
An OpenSearch cluster reachable over HTTP (see
[`examples/opensearch-community`](../opensearch-community) for a one-line Docker cluster on
9201):
...
```sh
cd examples/opensearch-community-2
go run .                          # connects to localhost:9201, seeds demo indices + catalog, serves :1433
...
with the columns already known to OpenSearch, primary key set to `id` when that field
exists, and foreign keys left empty for you to declare. With `--db` blank it seeds three
demo indices (`customers`, `products`, `orders`) with explicit mappings and the catalog that
declares their PKs and the two FK edges from `orders`.
...
The catalog is just data — one document per table in `haystak_catalog`, keyed by table name.
Columns come from the mapping, so a catalog document holds only `primary_key` and
`foreign_keys`:
...
```sh
curl -XPOST 'http://localhost:9201/haystak_catalog/_update/events?refresh=true' \
...
| column | `_mapping` field (OpenSearch type → SQL type); `_id` stays hidden |
...
It's a thin backend (`Caps{Pushdown, Writable, DDL}`). OpenSearch has no native
multi-database concept, so it exposes a single logical database (the cluster) and does not
implement `Databaser`. An index without a catalog document is not a SQL table — the declared
catalog is authoritative for the table set.

## Separate Go module

Its own `go.mod` (requiring `github.com/opensearch-project/opensearch-go/v2`, Apache-2.0)
with a `replace` to the parent keeps the core library dependency-free.

```sh
cd examples/opensearch-community-2 && go build ./... && go test ./...   # the test skips if OpenSearch is down
```
