...
// Package opensearch is a haystak-tds-spi backend over OpenSearch: indices map to SQL tables,
// document fields to columns (inferred by sampling _source), and writes/DDL to native OpenSearch APIs.
package opensearch
...
// Backend serves a set of OpenSearch indices as a SQL catalog. It is a thin (Scanner) backend, so
...
func New(client *osgo.Client, pattern string) *Backend {
	return &Backend{client: client, pattern: pattern, sample: 100}
}
...
// indexNames resolves the configured pattern to concrete index names, skipping system (dot) indices.
func (b *Backend) indexNames(ctx context.Context) ([]string, error) {
	res, err := b.client.Cat.Indices(
		...
		b.client.Cat.Indices.WithIndex(b.pattern),
	...
	if res.IsError() {
		return nil, fmt.Errorf("opensearch cat indices %q: %s", b.pattern, res.String())
	}
	var rows []struct {
...
func (b *Backend) CreateTable(ctx context.Context, t *catalog.Table) error {
	res, err := b.client.Indices.Create(t.Name, b.client.Indices.Create.WithContext(ctx))
	if err != nil {
		return err
	}
	defer res.Body.Close()
...
func (b *Backend) DropTable(ctx context.Context, table string) error {
	...
	defer res.Body.Close()
	if res.IsError() {
		return fmt.Errorf("opensearch delete index %s: %s", table, res.String())
	}
	return nil
}
