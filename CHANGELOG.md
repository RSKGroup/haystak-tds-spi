...
- Window functions `ROW_NUMBER`, `RANK`, `DENSE_RANK`, `NTILE`, `LAG`, `LEAD`, `FIRST_VALUE`, `LAST_VALUE` with `OVER (PARTITION BY … ORDER BY …)`. (`FIRST_VALUE`/`LAST_VALUE` use the whole partition as the frame; aggregate windows like `SUM(x) OVER (…)` and explicit frames are not yet supported.)
